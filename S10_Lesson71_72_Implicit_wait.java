...
public class S10_Lesson71_72_Implicit_wait {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		driver.manage().timeouts().implicitlyWait(5,
				TimeUnit.SECONDS); /*
									 * IMPLICIT Wait being globally defined here. This means that for every line
									 * that is being executed it will wait 5 seconds. unless the DOM loads prior to
									 * the 5 seconds.
									 */
		 ...
		 * waits.
		 * 
		 * Implicit Wait: -Globally defines wait by number of seconds before you throw
		 * an expcetion. -Will adhear to DOM were if page loads before set wait time,
		 * then the next function can be executed earlier than set time. When would you
		 * use this? -when all load times are known - a problem exisits where your wait
		 * times are too short for a specific scenario while another scenario may take
		 * longer. Lession 72 Explicit Wait: - can be targeted with a specific scenario
		 ...
		 * 
		 * ************************************* Implicit Wait Advantages: Pros:
		 * -Readable code/cleaner look as you dont have to define the wait times at the
		 * Line level.
		 * 
		 * Cons - You can possibly hide performance issues.
		 * 
		 */
		...
		driver.findElement(By.xpath("//button[contains(text(),'PROCEED TO CHECKOUT')]")).click(); // given that the text
																									...
																									// xpath.
		/*
		 * becuase the IMPLICIT WAIT was defined globally in Line 21 above the next line
		 * will succeed. Otherwise we know that the promocode will illicit a
		 * "cannot find element"
		 */
		...
		driver.findElement(By.cssSelector("button.promoBtn")).click(); // this function will also require a wait that we
																		// already added with the Implicit Wait at Line
																		// 21
		// check if message is properly displayed after the "Apply" button is pressed.
		System.out.println(driver.findElement(By.cssSelector("span.promoInfo")).getText()); // Expected result is "Promo
																							// code applied
																							// successfully!"
