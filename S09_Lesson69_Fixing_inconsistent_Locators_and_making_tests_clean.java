...
public class S09_Lesson69_Fixing_inconsistent_Locators_and_making_tests_clean {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		/*
		 * PROBLEM: In the Below example we know that the code works, However when we
		 * expected the 3 items "Cucumber","Brocolli" and "Beetroot" in the cart we got
		 * "Brocolli","Beetroot" and "Tomato"
		 * 
		 * REASON: The "Add to Cart" button is counted up in the index as it is
		 * displayed, However this button will change to "Added" when it is clicked,
		 * then retruned to "Add to Cart". It did not stay static Our code below then
		 * added the "Add to Cart" index starting from the "Cauliflower" rather than the
		 * "Brocolli"
		 * 
		 * TAKE AWAY: Do not rely on "TEXT" when qualifying locators.
		 * 
		 * FIX: Redefine the element in line 76 From:
		 * "driver.findElements(By.xpath("//button[text()='ADD TO
		 * CART']")).get(i).click();" To:
		 * "driver.findElements(By.xpath("//div[@class='product-action']/
		 * button")).get(i).click();"
		 * 
		 * 
		 */
