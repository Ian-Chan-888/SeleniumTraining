...
public class S13_Lesson096_TO_99_How_to_handle_table_Grids_in_webpage {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		/*
		 * Includes: 97. Techniues used for table grid_real time example. 98. Practise
		 * exercises on Tables - 1 99. Practise excercises on Tables - 2
		 * 
		 * Handling HTML5 tables (cricbuzz.com/live-scores/...Scorecard) 1. get a list
		 * of all the runs scored "R" and match it with the total runs. Problem is that
		 * there are several tables using the same identifiers. 2. Strategy: utilize
		 * loops and decoupling hardcoding concepts - There are common css within the
		 * values of the table. - subsequent tables need to be filtered out. as they may
		 * also have common css identifiers. - Look at Parent objects to help you
		 * landmark the correct identifiers. Then use Parent / child css concepts to
		 * isolate your references. - Parrent css - then Child css. Parrent class =
		 * cb-col cb-col-100 cb-ltst-wgt-hdr -defined by 'table' - 'div:nith-child(3)' =
		 * 3rd child of div tag in question. Using this syntax will further help define
		 * the row you are isolating. - 'get(i)' is the method for the index of i
		 * 
		 * ROW 3rd Col div[class='cb-col cb-col-100 cb-scrd-itms'] div:nth-child(3)
		 * 
		 * 3. Tables - 1 : - output the "Extras" and Total at the end of the tables. -
		 * Strategy : reference from Section 6 Lesson 37 - Traversing back to parent
		 * node from siblings.
		 * 
		 * 4. Tables - 2 : - Summing up all the values.
		 * 
		 */
		...
		WebElement teamNames = driver
				.findElement(By.cssSelector("div[class='cb-mtch-lst cb-col cb-col-100 cb-tms-itm'] "
		...
		driver.findElement(
				By.xpath("//div[contains(@class,'cb-col-67 cb-col cb-left cb-schdl')]//div[1]//div[1]//nav[1]//a[2]"))
				.click();

		Thread.sleep(2000);
		WebElement table = driver.findElement(By.cssSelector("div[class='cb-col cb-col-100 cb-ltst-wgt-hdr']"));

		// -this will grab all the row elements of the row and give you the count of
		// them
		@SuppressWarnings("unused")
		int rowcount = table.findElements(By.cssSelector("div[class='cb-col cb-col-100 cb-scrd-itms']")).size();// this
																												// statement
																												// scope
																												// is
																												// restriced
		...
		// -this will give you the count of the 3rd column and give you the count of
		// them
		...
		System.out.println("The UI representation of Extras= "
		...
		System.out.println("The UI representation of Total= "
				+ driver.findElement(By.xpath("//div[text()='Total']/following-sibling::div")).getText());

		// 4
		/*
		 * Get sum of all the values. We declare a sum variable and count the rows in
		 * the table.
		 */
		int sum = 0;
		int countsum = table
